...
export function AdminShell({ children }) {
    ...
    return (_jsxs("div", { className: "min-h-screen bg-[var(--color-surface)] text-[var(--color-text)]", children: [_jsxs("div", { className: "mx-auto flex min-h-screen w-full max-w-[1600px]", children: [_jsxs("aside", { className: cn("hidden border-r border-[var(--color-border-subtle)] bg-[var(--color-surface-raised)]/94 backdrop-blur lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col", collapsed ? "lg:w-20" : "lg:w-72"), children: [_jsxs("div", { className: "flex items-center justify-between gap-3 px-4 py-5", children: [!collapsed ? (_jsx(AdminBrand, {})) : (_jsx("div", { className: "mx-auto", children: _jsx(AdminBrand, { compact: true, showName: false }) })), _jsx(Tooltip, { content: collapsed ? "Expand sidebar" : "Collapse sidebar", children: _jsx(Button, { variant: "ghost", size: "sm", className: cn("shrink-0", collapsed && "mx-auto"), onClick: () => setCollapsed((current) => !current), "aria-label": collapsed ? "Expand sidebar" : "Collapse sidebar", children: collapsed ? (_jsx(PanelLeftOpen, { className: "h-4 w-4" })) : (_jsx(PanelLeftClose, { className: "h-4 w-4" })) }) })] }), _jsx("nav", { className: "admin-nav-scroll mt-1 flex-1 overflow-y-auto px-3 pb-5", children: _jsx("div", { className: "space-y-4", children: navigationSections.map((section) => (_jsxs(Fragment, { children: [!collapsed ? (_jsx("div", { className: "px-3", children: _jsx("p", { className: "text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-[var(--color-text-disabled)]", children: section.title }) })) : null, _jsx("div", { className: "space-y-1.5", children: section.items.map((item) => {
                                                    const Icon = item.icon;
                                                    return (_jsx(Tooltip, { content: collapsed ? item.label.toLowerCase() : item.description, className: collapsed ? "w-full" : undefined, children: _jsxs(NavLink, { to: item.to, end: item.to === "/admin", className: ({ isActive }) => cn("flex min-h-12 items-center gap-3 rounded-2xl px-3.5 py-3 text-sm font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]", collapsed && "justify-center px-0", isActive
