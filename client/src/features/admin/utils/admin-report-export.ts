...
type ExportBooking = AdminReportResponse["exportBookings"][number];
type ConciseExportRow = Record<(typeof csvHeaders)[number], string | number>;
type BookingTypeSummaryItem = AdminReportResponse["bookingTypeSummary"][number];
type StatusSummaryItem = AdminReportResponse["statusSummary"][number];
type ServiceSummaryItem = AdminReportResponse["serviceSummary"][number];
...
function getAppliedOption(row: ExportBooking) {
  return (
    row.appliedOption ??
...
function formatExportDate(row: ExportBooking) {
  if (row.slotStartsAt) {
...
function formatExportTime(row: ExportBooking) {
  if (row.slotStartsAt && row.slotEndsAt) {
    return `${timeFormatter.format(new Date(row.slotStartsAt))} - ${timeFormatter.format(
      new Date(row.slotEndsAt),
    )}`;
  }
...
function getExportBookings(report: AdminReportResponse): ExportBooking[] {
...
function getBookingTypeSummary(report: AdminReportResponse): BookingTypeSummaryItem[] {
  return Array.isArray(report.bookingTypeSummary) ? report.bookingTypeSummary : [];
}

function getStatusSummary(report: AdminReportResponse): StatusSummaryItem[] {
  return Array.isArray(report.statusSummary) ? report.statusSummary : [];
}

function getServiceSummary(report: AdminReportResponse): ServiceSummaryItem[] {
  return Array.isArray(report.serviceSummary) ? report.serviceSummary : [];
}

function buildBookingRows(report: AdminReportResponse): ConciseExportRow[] {
...
function buildVisualBar(value: number, maxValue: number) {
  if (maxValue <= 0) {
    return "";
  }
...
function applyDataRowStyle(row: Row, rowIndex: number) {
  row.height = 22;
...
function setSectionTitle(cell: Cell) {
  cell.font = {
    bold: true,
    color: { argb: "A58428" },
    size: 11,
...
function toAnalyticsRows(report: AdminReportResponse) {
...
async function loadWorkbookConstructor(): Promise<WorkbookConstructor> {
  const exceljsModule = await import("exceljs");
  const workbookConstructor =
    ("Workbook" in exceljsModule ? exceljsModule.Workbook : undefined) ??
...
async function createAdminWorkbook(report: AdminReportResponse) {
  ...
  summarySheet.getCell("A2").value = `${report.range.label} • Generated ${dateFormatter.format(
  ...
  applyMetricBlock(
    ...
    "A7:C9",
    "Amount Paid",
    formatCurrency(summary.paidValueKobo),
  );
  applyMetricBlock(
    summarySheet,
    "D7:F9",
  ...
  summarySheet.getCell("D14").value = `Quoted: ${
    topService ? formatCurrency(topService.quotedValueKobo) : formatCurrency(0)
  }`;
  summarySheet.getCell("D15").value = `Paid: ${
    topService ? formatCurrency(topService.paidValueKobo) : formatCurrency(0)
  }`;
...
export function downloadAdminReportCsv(report: AdminReportResponse) {
...
export async function downloadAdminReportWorkbook(report: AdminReportResponse) {
...
export async function buildAdminReportWorkbookBuffer(report: AdminReportResponse) {
  const workbook = await createAdminWorkbook(report);
